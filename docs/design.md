...
1. 接入 Prisma，把会话、消息、视觉摘要和指标写入 PostgreSQL。
2. 增加帧差检测和请求频率限制。
3. 补充接口测试和端到端演示脚本。
...
前端在手动画面分析成功后，会把最新 `VisionSummary` 通过 Realtime data channel 注入对话上下文：

- 使用 `conversation.item.create` 创建一条 system message。
- system message 包含视觉摘要、detail 档位和分析时间。
- 当用户询问画面、镜头、物体、环境或“你看到了什么”时，模型会优先使用最新视觉摘要回答。
- 如果视觉摘要先于 Realtime data channel 准备好，客户端会排队等待通道打开后同步。
- 右侧视觉摘要面板会展示“未同步、等待同步、已同步、同步失败”等状态，便于演示和排错。

自动抽帧、更完整的视觉问题识别和实时上下文更新策略会在后续 PR 中继续完善。
